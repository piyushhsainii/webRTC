    ...
    socket.onmessage = async (e) => {
      ...
      switch (message.type) {
        case OFFER:
          ...
          await receiverPc.setRemoteDescription(new RTCSessionDescription(message.payload.sdp));
    ...
    <div className="text-black">
      ...
      <div className='flex'>
        <div className="border-black border w-[300px] h-[300px]  flex flex-col justify-evenly  ">
          ...
          {ShowCam === true ? (
            <div className='cursor-pointer' onClick={() => setShowCam(false)}>
              <VideoOff color="#2e2e2e" />
            </div>
          ) : (
            <div className='cursor-pointer' onClick={() => setShowCam(true)}>
              <Video color="#2e2e2e" />
            </div>
          )}
        </div>
      ...
      <div className='cursor-pointer' onClick={sendVideo}>
        share Video
      </div>
    </div>
  );
